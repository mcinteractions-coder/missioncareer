...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [{ title: "Admin — Mission Career" }, { name: "robots", content: "noindex" }],
  }),
  component: AdminPage,
});
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-hero px-4">
        ...
        >
          ...
          <button className="mt-4 w-full rounded-full bg-gradient-primary py-3 font-semibold text-primary-foreground shadow-soft">
            Unlock
          </button>
          <Link
            to="/"
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const b = payload.new as Record<string, string>;
          addToast({
            type: "booking",
            title: "New booking confirmed",
            message: `${b.full_name} · ${b.slot_date} at ${b.slot_time}`,
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "leads" }, (payload) => {
        ...
        addToast({
          ...
          message: `${l.full_name}${l.country ? " from " + l.country : ""}`,
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`pointer-events-auto bg-card border shadow-card rounded-xl p-4 min-w-[280px] max-w-sm transition-all duration-300 ${
            t.type === "booking" ? "border-emerald-500/30" : "border-amber-500/30"
          }`}
        >
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="text-xs text-muted-foreground mt-1 truncate">{t.message}</div>
              <div className="text-[10px] text-muted-foreground mt-1">{timeAgo(t.created_at)}</div>
            </div>
            <button
      ...
      <header className="sticky top-0 z-20 bg-card border-b border-border">
        <div className="container mx-auto px-4 md:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link
              to="/"
              className="h-9 w-9 rounded-full bg-secondary flex items-center justify-center hover:bg-accent transition"
            >
          ...
          <button
            onClick={() => {
              ...
              sessionStorage.removeItem("mc_admin_pin");
              location.reload();
            }}
            ...
            <div key={p.id} className="bg-card rounded-2xl shadow-card p-4 flex gap-3">
              {isSuccess ? (
                ...
                p.image ? (
                  ...
                  <div className="h-12 w-12 rounded-full bg-gradient-primary text-primary-foreground font-bold flex items-center justify-center flex-shrink-0">
                    {p.title.charAt(0)}
                  </div>
                )
              ) : p.image ? (
...
function EditModal({
  ...
  const [gender, setGender] = useState<"male" | "female">(
    (post.gender as "male" | "female") || "male",
  );
  ...
  const onFile = async (f?: File) => {
    if (!f) return;
    if (f.size > 2_500_000) {
      alert("Image too large. Use under 2.5MB.");
      return;
    }
      ...
      >
        ...
        {isSuccess ? (
          <>
            ...
            <Field label="Indian College">
              <input
                value={prevCollege}
                onChange={(e) => setPrevCollege(e.target.value)}
                className="w-full rounded-xl border border-input bg-background px-3 py-2"
              />
            </Field>
            ...
            <Field label="Stars (1-5)">
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((n) => (
                  ...
                  >
                    <span className={n <= rating ? "text-amber-500" : "text-muted-foreground/40"}>
                      ★
                    </span>
                  </button>
                ))}
        ...
        <div className="flex gap-2 pt-2">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 rounded-full border border-input py-2.5 font-semibold"
          >
            Cancel
          </button>
          <button
            disabled={busy}
            className="flex-1 rounded-full bg-gradient-primary py-2.5 font-semibold text-primary-foreground disabled:opacity-60"
          >
            {busy ? "Saving…" : "Save"}
          </button>
        </div>
...
function PostForm({
    ...
    try {
      await addFn({
        data: {
          ...
          text:
            text.trim() ||
            (isSuccess ? `${title.trim()} — placed at ${university || destination}` : ""),
          image,
  ...
  return (
    <form
      onSubmit={submit}
      className="bg-card rounded-2xl shadow-card p-6 space-y-4 h-fit lg:sticky lg:top-32"
    >
      <h2 className="font-bold text-lg">Add new {kind}</h2>
      <div>
        <label className="text-sm font-semibold">
          {isSuccess || isReview || isAdmit ? "Name" : "Title"}
        </label>
        <input
          value={title}
          onChange={(e) => setTitle(e.target.value)}
      ...
      {isSuccess ? (
        <>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-sm font-semibold">
                Flag Code <span className="text-muted-foreground font-normal">(2 letters)</span>
              </label>
              <input
                value={flagCode}
                onChange={(e) => setFlagCode(e.target.value)}
          ...
          <div>
            <label className="text-sm font-semibold">
              Previous Course <span className="text-muted-foreground font-normal">(optional)</span>
            </label>
            <input
              value={prevCourse}
              onChange={(e) => setPrevCourse(e.target.value)}
          ...
          <div>
            <label className="text-sm font-semibold">
              Previous College <span className="text-muted-foreground font-normal">(optional)</span>
            </label>
            <input
              value={prevCollege}
              onChange={(e) => setPrevCollege(e.target.value)}
          ...
          <div>
            <label className="text-sm font-semibold">
              Gender <span className="text-muted-foreground font-normal">(for default avatar)</span>
            </label>
      ...
      <div>
        <label className="text-sm font-semibold">
          {isSuccess ? "Student Photo (optional)" : "Image"}
        </label>
      ...
      <button
        disabled={busy}
        className="w-full rounded-full bg-gradient-primary py-3 font-semibold text-primary-foreground shadow-soft hover:shadow-glow transition disabled:opacity-60"
      >
        {busy ? "Saving…" : "Publish"}
      </button>
    </form>
              ...
              <div className="mt-3 grid sm:grid-cols-2 gap-2 text-sm">
                {l.phone && (
                  <div>
                    ...
                    <a href={`tel:${l.phone}`} className="text-primary font-medium">
                      {l.phone}
                    </a>
                  </div>
                )}
                {l.email && (
                  <div>
                    ...
                    <a href={`mailto:${l.email}`} className="text-primary font-medium">
                      {l.email}
                    </a>
                  </div>
                )}
                {l.country && (
                  <div>
                    <span className="text-muted-foreground">Country:</span> {l.country}
                  </div>
                )}
                {l.study_level && (
                  <div>
                    <span className="text-muted-foreground">Level:</span> {l.study_level}
                  </div>
                )}
              </div>
              {l.message && (
            ...
            <div key={date}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-muted-foreground">
                  · {groups[date].length} {groups[date].length === 1 ? "booking" : "bookings"}
                </span>
                {date === todayISO && (
                  <span className="text-[10px] uppercase tracking-wide bg-primary text-primary-foreground rounded-full px-2 py-0.5 font-bold">
                    Today
                  </span>
                )}
              </div>
              <div className="grid md:grid-cols-2 gap-3">
                {groups[date].map((b) => (
                  <div key={b.id} className="bg-card rounded-2xl shadow-card p-4">
                    ...
                    {b.notes && (
                      <p className="mt-3 text-xs text-foreground whitespace-pre-wrap border-t border-border pt-2">
                        {b.notes}
                      </p>
                    )}
                  </div>
...
function AiInsightsCard({ hours }: { hours: number }) {
  const [insights, setInsights] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [err, setErr] = useState("");
  const fn = useServerFn(adminAiInsights);
  const pin = sessionStorage.getItem("mc_admin_pin") || "";
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setInsights((r as { insights: string }).insights);
    } catch (e) {
    ...
    <div className="bg-card rounded-2xl shadow-card p-5 border border-primary/20">
      <div className="flex items-center justify-between gap-3 mb-3">
        <h3 className="font-bold text-base flex items-center gap-2">
          <Zap className="h-4 w-4 text-amber-500" /> AI Smart Insights
        </h3>
        <button
          onClick={load}
          disabled={loading}
      ...
      {!insights && !err && loading && (
        <p className="text-sm text-muted-foreground">AI is analyzing your traffic…</p>
      )}
      {insights && (
...
function FunnelCard({ funnel }: { funnel: AnalyticsData["funnel"] }) {
  const steps = [
    { label: "Unique Visitors", value: funnel.visitors, color: "from-blue-500 to-blue-400" },
    { label: "Leads", value: funnel.leads, color: "from-amber-500 to-amber-400" },
    { label: "Bookings", value: funnel.bookings, color: "from-emerald-500 to-emerald-400" },
  ];
    ...
    <div className="bg-card rounded-2xl shadow-card p-5">
      ...
      <div className="space-y-3">
        {steps.map((step, i) => {
          ...
          const conversion = i === 0 ? 100 : prev > 0 ? Math.round((step.value / prev) * 100) : 0;
          return (
            <div key={step.label}>
      ...
      <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-border">
        <div className="text-center">
          <div className="text-xs text-muted-foreground">Visitor → Lead</div>
          <div className="text-lg font-extrabold text-amber-500">
            {(funnel.leadRate * 100).toFixed(1)}%
          </div>
        </div>
        <div className="text-center">
          <div className="text-xs text-muted-foreground">Visitor → Booking</div>
          <div className="text-lg font-extrabold text-emerald-500">
            {(funnel.bookingRate * 100).toFixed(1)}%
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <StatCard
          icon={Users}
          label="Live now"
          value={data.totals.liveVisitors}
          ...
          <div className="flex items-end gap-1 h-32">
            {data.byHour.map((h) => (
              <div key={h.k} className="flex-1 flex flex-col items-center gap-1 group">
                <div
                  className="w-full bg-gradient-primary rounded-t opacity-80 group-hover:opacity-100 transition"
                  style={{ height: `${(h.v / maxHour) * 100}%`, minHeight: "2px" }}
                  title={`${h.k}:00 — ${h.v} views`}
                />
              </div>
            ))}
          </div>
      ...
      <div className="bg-card rounded-2xl shadow-card p-5">
        <h3 className="font-bold text-base mb-3 flex items-center gap-2">
          <CalendarDays className="h-4 w-4 text-primary" /> Recent Booking Form Submissions (
      ...
      <div className="bg-card rounded-2xl shadow-card p-5">
        <h3 className="font-bold text-base mb-3 flex items-center gap-2">
          <Inbox className="h-4 w-4 text-primary" /> Recent Lead Form Submissions (
      ...
      <div className="bg-card rounded-2xl shadow-card p-5">
        ...
        <div className="max-h-96 overflow-y-auto divide-y divide-border">
          {data.recentEvents.slice(0, 60).map((e) => (
            <div key={e.id} className="py-2 flex items-center gap-3 text-xs">
              ...
              <span className="w-32 shrink-0 truncate">
                {flag(e.country)} {[e.city, e.country].filter(Boolean).join(", ") || "Unknown"}
              </span>
          ...
          {data.recentEvents.length === 0 && (
            <p className="text-xs text-muted-foreground py-3">No visitor events yet.</p>
          )}
        </div>
      </div>
    </div>
...
function StatCard({
  ...
  return (
    <div className="bg-card rounded-2xl shadow-card p-4">
      ...
      <div
        className={`mt-2 text-3xl font-extrabold ${accent || "text-foreground"} ${pulse && value > 0 ? "animate-pulse" : ""}`}
      >
        {value}
      </div>
    </div>
...
function BarList({
  ...
  render?: (k: string) => React.ReactNode;
  mono?: boolean;
}) {
        ...
        <div className="space-y-1.5">
          {items.slice(0, 10).map((it) => (
            <div key={it.k} className="relative">
              <div
                className="absolute inset-0 bg-primary-soft rounded"
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <h3 className="font-bold text-base flex items-center gap-2">
          ...
          <span className="text-xs font-normal text-muted-foreground">
            · Click any row for full journey + AI summary
          </span>
        </h3>
        <div className="flex gap-2">
          {(["all", "converted", "anon"] as const).map((k) => (
            ...
            >
              {k === "all" ? "All" : k === "converted" ? "Filled form ✨" : "Anonymous"}
            </button>
          ))}
        </div>
      </div>
            ...
            <tbody>
              {shown.map((r) => (
                ...
                >
                    ...
                    {r.identity ? (
                      <div className="min-w-0">
                        <div className="font-semibold text-foreground flex items-center gap-1.5">
                          {r.identity.name}
                          <span
                            className={`text-[9px] uppercase tracking-wide rounded-full px-1.5 py-0.5 font-bold ${r.identity.type === "booking" ? "bg-primary text-primary-foreground" : "bg-amber-500/20 text-amber-700 dark:text-amber-300"}`}
                          >
                            {r.identity.type}
                          </span>
                        </div>
                  ...
                  <td className="py-2.5 px-2 text-xs">
                    {r.pageviews} <span className="text-muted-foreground">({r.unique_pages}u)</span>
                  </td>
...
function SessionDetailModal({ row, onClose }: { row: SessionRow; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="bg-card rounded-2xl shadow-card w-full max-w-2xl my-8"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between p-5 border-b border-border">
          <div className="min-w-0">
            <h3 className="font-extrabold text-lg flex items-center gap-2">
              ...
              {row.identity && (
                <span
                  className={`text-[10px] uppercase rounded-full px-2 py-0.5 font-bold ${row.identity.type === "booking" ? "bg-primary text-primary-foreground" : "bg-amber-500/20 text-amber-700 dark:text-amber-300"}`}
                >
                  {row.identity.type}
                </span>
              )}
        ...
        <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          {!data && !err && (
            <p className="text-sm text-muted-foreground">Loading detail & generating AI summary…</p>
          )}
          ...
          {data && (
            <>
              ...
              <div className="rounded-2xl border border-primary/30 bg-primary-soft/40 p-4">
                ...
                {data.ai_summary ? (
                  ...
                  <p className="text-xs text-muted-foreground italic">
                    No summary available (not enough activity or AI unavailable).
                  </p>
                )}
              </div>
                    ...
                    <>
                      ...
                      {data.booking.email && (
                        <p>
                          ...
                          <a href={`mailto:${data.booking.email}`} className="text-primary">
                            {data.booking.email}
                          </a>
                        </p>
                      )}
                      {data.booking.country && (
                        <p>
                          <Globe2 className="inline h-3.5 w-3.5 mr-1 text-muted-foreground" />{" "}
                          {data.booking.country}
                        </p>
                      )}
                      <p>
                        <CalendarDays className="inline h-3.5 w-3.5 mr-1 text-muted-foreground" />{" "}
                        {data.booking.slot_date} · {data.booking.slot_time} · {data.booking.mode}
                      </p>
                      {data.booking.notes && (
                        <p className="text-xs text-muted-foreground border-t border-border pt-2 mt-2 whitespace-pre-wrap">
                          📝 {data.booking.notes}
                        </p>
                      )}
                    </>
                  ...
                  {data.lead && !data.booking && (
                    <>
                      {data.lead.phone && (
                        <p>
                          <Phone className="inline h-3.5 w-3.5 mr-1 text-primary" />
                          <a href={`tel:${data.lead.phone}`} className="text-primary font-semibold">
                            {data.lead.phone}
                          </a>
                        </p>
                      )}
                      {data.lead.email && (
                        <p>
                          <Mail className="inline h-3.5 w-3.5 mr-1 text-primary" />
                          <a href={`mailto:${data.lead.email}`} className="text-primary">
                            {data.lead.email}
                          </a>
                        </p>
                      )}
                      ...
                      {data.lead.study_level && (
                        <p className="text-xs text-muted-foreground">
                          Study level: {data.lead.study_level}
                        </p>
                      )}
                      {data.lead.message && (
                        <p className="text-xs text-muted-foreground border-t border-border pt-2 mt-2 whitespace-pre-wrap">
              ...
              <div className="text-[10px] text-muted-foreground border-t border-border pt-3 break-all">
                <div>
                  Session ID: <span className="font-mono">{row.session_id}</span>
                </div>
                {data.summary.user_agent && (
                  <div className="mt-1">UA: {data.summary.user_agent}</div>
                )}
              </div>
            </>
          )}
